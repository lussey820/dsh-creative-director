 ...
 * 规则：无命中→low；跨领域同时命中或并列→low（保守，防止误判）；命中<2 个→low。
...
export function classifyDomain(request, profiles) {
  ...
  const hitters = results.filter((r) => r.hits.length > 0)
  if (hitters.length === 0) {
    return { mode: null, confidence: 'low', reason: '未命中任何领域关键词，需向用户确认领域' }
  }
  const best = hitters.reduce((a, b) => (b.hits.length > a.hits.length ? b : a))
  const tied = hitters.filter((r) => r.hits.length === best.hits.length).length > 1
  const crossed = hitters.length > 1 // 跨领域同时命中 → 保守降级，交给模型/用户确认
  const confidence = tied || crossed || best.hits.length < 2 ? 'low' : 'high'
